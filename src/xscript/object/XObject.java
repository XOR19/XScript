...
public final class XObject{
	...
	public XObject(int pointer, ObjectInput in) throws IOException {
		...
		fields = new HashMap<String, XValue>(count<DEFAULT_CAPACITY?DEFAULT_CAPACITY:count);
		while(count>0){
	...
	public void setTypeData(XRuntime runtime, XTypeData data){
		if(objectData!=null)
			throw new IllegalStateException();
		if(runtime.getObject(type)==this || type.equals(runtime.getBaseType(XUtils.TYPE))){
			objectData = data;
		}else{
			throw new IllegalStateException();
		}
	...
	public XValue getWeakRef(XRuntime runtime){
		if(weakRef==XValueNull.NULL){
			weakRef = runtime.alloc(runtime.getBaseType(XUtils.WEAK_REF), new XValueObj(pointer));
		}
		return weakRef;
	}
