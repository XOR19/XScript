...
public class XUtils {
				...
				for(XValue t:classes){
					if(found){
						...
						if(v instanceof XValueAttr){
							...
							v = typeData.getAttr(runtime, value, obj, ((XValueAttr)v).getAttrID());
	...
	public static XValue lookup(XRuntime runtime, XValue value, String attr, int addRef){
		XValue v = lookupTry(runtime, value, attr, addRef);
		if(v==null){
			throw new XRuntimeException("TypeError", "Attribute '%s' not found", attr);
		}
		return v;
	}
	
	public static XValue set(XRuntime runtime, XValue value, String attr, XValue v, int addRef){
		if(v==null)
	...
	public static XValue del(XRuntime runtime, XValue value, String attr, int addRef){
				...
				for(XValue t:classes){
					if(found){
						...
						if(v instanceof XValueAttr){
							obj = runtime.getObject(t);
							typeData = (XTypeData)obj.getData();
							v = typeData.delAttr(runtime, value, obj, ((XValueAttr)v).getAttrID());
							break;
						}else if(v!=null){
							return null;
						}
	...
	public static XValue getDeclaring(XRuntime runtime, XValue value, String attr, int addRef){
		XValue v = value.getRaw(runtime, attr);
		if(v!=null && !(v instanceof XValueAccess || v instanceof XValueAttr))
			return value;
		XValue type = value.getType(runtime);
		XObject obj = runtime.getObject(type);
		XTypeData typeData = (XTypeData)obj.getData();
	...
	public static boolean isOverwritten(XRuntime runtime, XValue value, String attr){
	...
	public static String getString(XRuntime rt, XValue value) {
		...
		return data instanceof XObjectDataString?((XObjectDataString)data).getString():null;
	}

	@SuppressWarnings("unchecked")
	public static <T> T getDataAs(XRuntime rt, XValue value, Class<T> c) {
		XObjectData data = getData(rt, value);
		return data!=null && c.isAssignableFrom(data.getClass())?(T)data:null;
	}
	...
	public static boolean isDerivedOf(XRuntime rt, XValue base, XValue type) {
		...
		if(typeObj==null || !typeObj.isType(rt) || baseObj==null || !baseObj.isType(rt))
			return false;
	...
	public static XValue wrap(XRuntime rt, Object obj){
		if(obj==null){
			...
			if(rt.getObject(object.getPointer())!=object){
				throw new IllegalArgumentException("XObject is form another Runtime");
			}
			...
			return rt.createList((List<XValue>)obj);
		}else if(obj instanceof Map){
			return rt.createMap((Map<String, XValue>)obj);
	...
	public static ScriptException getException(XRuntime rt, XValue exception){
		String type = XUtils.getDataAs(rt, exception.getType(rt), XTypeData.class).getName();
		...
		if(stackTrace!=null && stackTrace!=XValueNull.NULL){
			...
			if(list!=null){
				...
				int i=0;
				for(XValue v:list){
					String declaringPath = getString(rt, v.getRaw(rt, "_declaringPath"));
					String methodName = getString(rt, v.getRaw(rt, "_methodName"));
					String fileName = getString(rt, v.getRaw(rt, "_fileName"));
					int lineNumber = (int) v.getRaw(rt, "_lineNumber").getInt();
					jstackTrace[i++] = new StackTraceElement(declaringPath, methodName, fileName, lineNumber);
				}
			}
		}
		String fileName = jstackTrace==null||jstackTrace.length==0?null:jstackTrace[0].getFileName();
		int line = jstackTrace==null||jstackTrace.length==0?-1:jstackTrace[0].getLineNumber();
		ScriptException e;
		...
		if(jstackTrace!=null && jstackTrace.length!=0){
			...
			StackTraceElement[] nste = new StackTraceElement[ste.length+jstackTrace.length];
	...
	public static void throwException(XRuntime rt, XValue exception) throws ScriptException {
	...
	public static XValue convertStackTrace(XRuntime rt, StackTraceElement[] stackTrace) {
